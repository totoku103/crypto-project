...
public class KISA_SHA256 {
  ...
  public static final int SHA256_DIGEST_BLOCKLEN = 64;
  public static final int SHA256_DIGEST_VALUELEN = 32;
  ...
  private static final void FF(
      int[] abcdefgh, int a, int b, int c, int d, int e, int f, int g, int h, int[] X, int j) {
    long T1;
    ...
    abcdefgh[h] =
        (int)
            (T1
                + Common.intToUnsigned(Sigma0(abcdefgh[a]))
                + Common.intToUnsigned(Maj(abcdefgh[a], abcdefgh[b], abcdefgh[c])));
  }
  ...
  public static void SHA256_Process(SHA256_INFO Info, byte[] pszMessage, int uDataLen) {
    ...
    while (uDataLen >= SHA256_DIGEST_BLOCKLEN) {
      Common.arraycopy_offset(
          Info.szBuffer, 0, pszMessage, pszMessage_offset, SHA256_DIGEST_BLOCKLEN);
