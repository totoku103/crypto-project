...
public class Sha3 {
  ...
  private static final byte[] keccakState = new byte[KECCAK_STATE_SIZE];
  ...
  private static final int[] KECCAK_ROTC = {
    1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 2, 14,
    27, 41, 56, 8, 25, 43, 62, 18, 39, 61, 20, 44
  };

  private static final int[] KECCAK_PI_LN = {
  ...
  private static void keccak(byte[] state) {
    ...
    for (i = 0; i < 25; i++) {
      s[i][0] =
          ((0x0FF & state[i * 8 + 0])
              | ((0x0FF & state[i * 8 + 1]) << 8)
              | ((0x0FF & state[i * 8 + 2]) << 16)
              | ((0x0FF & state[i * 8 + 3]) << 24));
      s[i][1] =
          ((0x0FF & state[i * 8 + 4])
              | ((0x0FF & state[i * 8 + 5]) << 8)
              | ((0x0FF & state[i * 8 + 6]) << 16)
              | ((0x0FF & state[i * 8 + 7]) << 24));
    }
    ...
    for (round = 0; round < KECCAK_ROUND; round++) {
      ...
      /* Iota */
      s[0][0] ^= KECCAK_ROUND_CONSTANTS[round][0];
      s[0][1] ^= KECCAK_ROUND_CONSTANTS[round][1];
    }
  ...
  private static int keccakAbsorb(byte[] input, int inLen, int rate, int capacity) {
    int offset = 0;
    int iLen = inLen;
    ...
    offset = 0;
    while (iLen > 0) {
      if ((endOffset != 0) && (endOffset < rateInBytes)) {
        blockSize = (((iLen + endOffset) < rateInBytes) ? (iLen + endOffset) : rateInBytes);

        for (i = endOffset; i < blockSize; i++) keccakState[i] ^= input[i - endOffset];

        offset += blockSize - endOffset;
        iLen -= blockSize - endOffset;
      } else {
        blockSize = ((iLen < rateInBytes) ? iLen : rateInBytes);

        for (i = 0; i < blockSize; i++) keccakState[i] ^= input[i + offset];
  ...
  private static int keccakSqueeze(byte[] output, int outLen, int rate, int suffix) {
    int offset = 0;
    int oLen = outLen;
    int rateInBytes = rate / 8;
    int blockSize = endOffset;
    int i = 0;

    keccakState[blockSize] ^= suffix;

    if (((suffix & 0x80) != 0) && (blockSize == (rateInBytes - 1))) keccak(keccakState);
    ...
    while (oLen > 0) {
      ...
      for (i = 0; i < blockSize; i++) output[i + offset] = keccakState[i];

      offset += blockSize;
      oLen -= blockSize;

      if (oLen > 0) keccak(keccakState);
    }

    return SHA3_OK;
  }

  public final void sha3Init(int bitSize, int useSHAKE) {
    keccakCapacity = bitSize * 2;
    keccakRate = KECCAK_SPONGE_BIT - keccakCapacity;

    if (useSHAKE == SHA3_SHAKE_USE) keccakSuffix = KECCAK_SHAKE_SUFFIX;
    else keccakSuffix = KECCAK_SHA3_SUFFIX;

    for (int i = 0; i < KECCAK_STATE_SIZE; i++) keccakState[i] = 0;

    endOffset = 0;
  }

  public final int sha3Update(byte[] input, int inLen) {
    return keccakAbsorb(input, inLen, keccakRate, keccakCapacity);
  }

  public final int sha3Final(byte[] output, int outLen) {
    int ret = 0;

    ret = keccakSqueeze(output, outLen, keccakRate, keccakSuffix);
  ...
  public final int toHash(
      byte[] output, int outLen, byte[] input, int inLen, int bitSize, int useShake) {
    int ret = 0;

    if (useShake == SHA3_SHAKE_USE) {
      if ((bitSize != KECCAK_SHAKE128) && (bitSize != KECCAK_SHAKE256)) return SHA3_PARAMETER_ERROR;

      sha3Init(bitSize, SHA3_SHAKE_USE);
    } else {
      if ((bitSize != KECCAK_SHA3_224)
          && (bitSize != KECCAK_SHA3_256)
          && (bitSize != KECCAK_SHA3_384)
          && (bitSize != KECCAK_SHA3_512)) return SHA3_PARAMETER_ERROR;

      if ((bitSize / 8) != outLen) return SHA3_PARAMETER_ERROR;

      sha3Init(bitSize, SHA3_SHAKE_NONE);
    }

    sha3Update(input, inLen);

    ret = sha3Final(output, outLen);
