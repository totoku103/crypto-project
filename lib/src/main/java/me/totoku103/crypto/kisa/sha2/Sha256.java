...
public class Sha256 {
  ...
  private static void ff(
      int[] param, int a, int b, int c, int d, int e, int f, int g, int h, int[] X, int j) {
    long t1;

    t1 =
        Common.intToUnsigned(param[h])
            + Common.intToUnsigned(sigma1(param[e]))
            + Common.intToUnsigned(ch(param[e], param[f], param[g]))
    ...
    param[h] =
        (int)
            (t1
                + Common.intToUnsigned(sigma0(param[a]))
                + Common.intToUnsigned(maj(param[a], param[b], param[c])));
  }
  ...
  private static void transform(byte[] Message, int[] ChainVar) {
    ...
    for (j = 0; j < 16; j++) x[j] = getData(Message, j * 4);

    for (j = 16; j < 64; j++)
      x[j] =
          (int)
              (Common.intToUnsigned(rho1(x[j - 2]))
                  + Common.intToUnsigned(x[j - 7])
                  + Common.intToUnsigned(rho0(x[j - 15]))
  ...
  public static void close(Sha256Info Info, byte[] pszDigest) {
    ...
    Common.intToByteUnit(
        Info.szBuffer, ((int) (SHA256_DIGEST_BLOCK_LEN / 4 - 2)) * 4, Info.uHighLength, ENDIAN);
    Common.intToByteUnit(
        Info.szBuffer, ((int) (SHA256_DIGEST_BLOCK_LEN / 4 - 1)) * 4, Info.uLowLength, ENDIAN);

    transform(Info.szBuffer, Info.uChainVar);

    for (i = 0; i < SHA256_DIGEST_VALUE_LEN; i += 4) bigD2B((Info.uChainVar)[i / 4], pszDigest, i);
  }
  ...
  public static class Common {
    ...
    public static void copyArrayOffset(
