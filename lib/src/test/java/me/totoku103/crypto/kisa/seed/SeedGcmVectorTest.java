...
class SeedGcmVectorTest {
  ...
  static Stream<TestVector> testVectors() throws IOException {
    ...
    final URL resource =
        SeedGcmVectorTest.class.getResource("/testvectors/seedgcm/GCM_SEED-128_AE.txt");
    ...
    for (String line : lines) {
      ...
      } else if (line.startsWith("Key =")) current.key = getSplitAndValue(line);
      else if (line.startsWith("IV =")) current.iv = getSplitAndValue(line);
      else if (line.startsWith("PT =")) current.pt = getSplitAndValue(line);
      else if (line.startsWith("Adata =")) current.adata = getSplitAndValue(line);
      else if (line.startsWith("C =")) current.c = getSplitAndValue(line);
      else if (line.startsWith("T =")) current.t = getSplitAndValue(line);
    }
  ...
  public void vectorTest(TestVector testVector) {
    ...
    int decryptedLen =
        seedGcm.decryptionGcm(
            decryptedPt, ctWithTag, ctWithTag.length, macLen, iv, iv.length, aad, aad.length, key);
