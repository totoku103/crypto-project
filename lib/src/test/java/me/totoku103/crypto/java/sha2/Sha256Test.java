...
class Sha256Test {
  ...
  void isSha256Available() {
    assertTrue(
        Sha256.isSha256Available(), "SHA-256 should be available in a standard JDK environment");
  }
  ...
  void sha256Hash_abcString() {
    ...
    final int result =
        sha256.sha256Hash(outputBuffer, outputBuffer.length, plainBytes, plainBytes.length);
